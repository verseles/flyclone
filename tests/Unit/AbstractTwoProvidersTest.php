...
// Name for the right (destination) provider
    ...
    public function instantiate_with_two_providers(Provider $left_side, Provider $right_side): Rclone
    {
    ...
    public function touch_a_file_on_left_side(Rclone $two_sides_rclone): array
    {
        ...
        $temp_filepath = $this->left_working_directory . '/flyclone_left_touch_' . $this->random_string();
    ...
    public function write_to_a_file_on_left_side(array $params): array
    {
    ...
    public function move_file_to_right_side(array $params): array
    {
        ...
        $new_place_on_right = $this->right_working_directory . '/' . basename($file_on_left_side);
        ...
        // Capture the result to check statistics
        $result = $two_sides_rclone->moveto($file_on_left_side, $new_place_on_right);
        ...
        if (isset($check_new_place_on_right->details->Size) && ! $two_sides_rclone->isRightSideDirAgnostic()) {
    ...
    public function delete_file_on_right_side(array $params): array
    {
    ...
    public function test_move_with_progress_between_providers(array $params): void
    {
        ...
        $largeFileSourcePathOnLeft = $this->left_working_directory . '/large_source_for_progress_' . $this->random_string() . '.dat';
        ...
        $destinationDirOnRightSide = $this->right_working_directory . '/progress_test_dest_dir_' . $this->random_string();
        ...
        self::assertFalse(
            $rcloneLeftOnly->is_file($largeFileSourcePathOnLeft)->exists,
