...
OUTPUT;
        ...
        self::assertEquals(1610612736, $stats->bytes); // 1.5 GiB
    ...
    #[Test]
    public function parse_output_with_errors(): void
    {
        $output = <<<'OUTPUT'
Transferred:        100 MiB / 200 MiB, 50%, 5 MiB/s, ETA 20s
Errors:                 5
Checks:                 2
...
OUTPUT;
        ...
        self::assertEquals(104857600, $stats->bytes); // 100 MiB
        self::assertEquals(10, $stats->files);
        self::assertEquals(5, $stats->errors);
        self::assertEquals(2, $stats->checks);
        self::assertEquals(10.0, $stats->elapsed_time);
    }
    ...
    #[Test]
    public function format_bytes(): void
    {
        self::assertEquals('0 B', StatsParser::formatBytes(0));
        self::assertEquals('1 KiB', StatsParser::formatBytes(1024));
        self::assertEquals('1.5 MiB', StatsParser::formatBytes(1572864));
    }

    #[Test]
    public function convert_size_to_bytes(): void
    {
        self::assertEquals(1024, StatsParser::convertSizeToBytes('1K'));
        self::assertEquals(1048576, StatsParser::convertSizeToBytes('1M'));
        self::assertEquals(1073741824, StatsParser::convertSizeToBytes('1G'));
        self::assertEquals(1536, StatsParser::convertSizeToBytes('1.5K'));
        self::assertEquals(1610612736, StatsParser::convertSizeToBytes('1.5 GiB'));
    }

    #[Test]
    public function convert_duration_to_seconds(): void
    {
        self::assertEquals(90.0, StatsParser::convertDurationToSeconds('1m30s'));
        self::assertEquals(3661.0, StatsParser::convertDurationToSeconds('1h1m1s'));
...
JSON;
    ...
    {
        // Simulating rclone outputting multiple JSON objects over time
        $output = <<<'JSON'
